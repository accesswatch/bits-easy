"""Logging shim for the Spellforge runtime.
...
All log messages are prefixed with ``Spellforge:`` so they are trivially
...
def _build_stdlib_logger(name: str) -> logging.Logger:
    ...
    if not _stdlib_configured:
        root = logging.getLogger("spellforge")
...
def get_logger(name: str = "spellforge") -> Any:
    ...
    loggers honour the hierarchy, so callers can pass ``"spellforge.engine"``
