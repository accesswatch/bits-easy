...
class _TextInfo:
    def __init__(self, text: str, start: int = 0, end: int = 0):
        ...
        self.bookmark = (start, end)


class _TreeInterceptor:
    def __init__(self, all_text: str, selected_text: str, caret: int, sel_start: int, sel_end: int):
...
class LiveAdapterTests(unittest.TestCase):
    def test_app_id_detection(self):
        ...
        self.assertEqual(app_id_from_focus_object(_FocusObject("code", "", "", 0, 0, 0)), "vscode")
        self.assertEqual(app_id_from_focus_object(_FocusObject("code-insiders", "", "", 0, 0, 0)), "vscode")
        self.assertEqual(app_id_from_focus_object(_FocusObject("notepad", "", "", 0, 0, 0)), "notepad")
    ...
    def test_snapshot_prefers_tree_interceptor_on_virtual_surface(self):
        focus = _FocusObject(
            app_name="chrome",
            all_text="focus-level text should not win",
            selected_text="",
            caret=0,
            sel_start=0,
            sel_end=0,
        )
        focus.treeInterceptor = _TreeInterceptor(
            all_text="virtual buffer text",
        ...
        snapshot = snapshot_from_focus_object(focus)
        self.assertEqual(snapshot.text, "virtual buffer text")
        self.assertEqual(snapshot.selected_text, "buffer")

    def test_interactive_role_uses_control_name_when_text_is_missing(self):
        focus = _FocusObject(
            app_name="chrome",
            all_text="",
            selected_text="",
        ...
        focus.role = "link"
        focus.name = "Open details"
        focus.value = ""

        snapshot = snapshot_from_focus_object(focus)
        self.assertEqual(snapshot.text, "Open details")
        self.assertEqual(snapshot.selected_text, "Open details")
    ...
    def test_word_live_adapter_prefers_selected_text(self):
        ...
        sel = adapter.normalize_range(ctx, start=6, end=11)
        self.assertEqual(sel.text, "bravo")
        self.assertEqual(sel.start, 6)
        self.assertEqual(sel.end, 11)
        self.assertEqual(sel.source_kind, "native-focus-selection")

    def test_interceptor_selection_source_kind_is_reported(self):
        focus = _FocusObject(
            app_name="chrome",
            all_text="ignore focus text",
            selected_text="",
            caret=0,
            sel_start=0,
            sel_end=0,
        ...
        focus.treeInterceptor = _TreeInterceptor(
            all_text="virtual buffer text",
            selected_text="virtual",
            caret=7,
            sel_start=0,
            sel_end=7,
        )
        adapter = BrowserLiveAdapter("chrome", snapshot_provider=lambda: snapshot_from_focus_object(focus))
        ctx = AppContext(
            app_id="chrome",
            window_id="1001",
            control_id="editor",
            buffer="virtual buffer text",
            caret=7,
            clipboard_text="",
        )
        sel = adapter.normalize_range(ctx, 0, 7)
        self.assertEqual(sel.source_kind, "native-interceptor-selection")
